...
function checkAutomatedSpam(customerName: string, text: string, fromNumber: string): { isSpam: boolean; reason: string } | null {
  ...
  // 1. Senders commonly associated with automated notification bots
  const automatedKeywords = [
    "serasa", "celpe", "compesa", "neoenergia", "spc brasil", "spc", "mercadolivre", "mercado livre",
    "mercadopago", "mercado pago", "nubank", "bradesco", "itau", "itaú", "santander", "caixa economica",
    "banco do brasil", "banco", "claro", "vivo", "tim", "oi", "coelba", "enel", "sabesp", "copasa",
    "sanepar", "embasa", "cagece", "saneago", "corsan", "cedae", "light", "ampla", "energisa",
    "elektro", "cpfl", "edp", "neoenergia celpe", "serasa experian", "serasa consumidor",
    "atendimento automático", "sistema", "notificação", "notificacao"
  ];
  ...
  for (const kw of automatedKeywords) {
    if (
      ...
      nameLower.includes(`${kw} `) || 
      nameLower.startsWith(kw) || 
      nameLower.endsWith(kw)
    ) {
      return { isSpam: true, reason: `Nome do remetente contém palavra de sistema automatizado ("${kw}")` };
    }
  }

  // 2. Specific keywords/patterns in the message text indicating system/bot notifications
  
  // A. Verification codes / One-time passwords (OTP)
  if (
    /c[oó]digo de verifica[cç][aã]o/i.test(textLower) ||
    /c[oó]digo de confirma[cç][aã]o/i.test(textLower) ||
    /c[oó]digo de seguran[cç]a/i.test(textLower) ||
    /token de seguran[cç]a/i.test(textLower) ||
    /seu c[oó]digo/i.test(textLower) ||
    /seu token/i.test(textLower) ||
    /use o c[oó]digo/i.test(textLower) ||
    /verification code/i.test(textLower) ||
    /seu pin/i.test(textLower) ||
    /c[oó]digo de acesso/i.test(textLower)
  ) {
    return { isSpam: true, reason: "Mensagem parece ser um código de verificação/segurança automatizado (OTP)" };
  }

  // B. Billing & Collections auto-notifications (Serasa, Celpe, etc.)
  ...
  // Check if message mentions billing companies/utilities and billing keywords
  const utilityTerms = ["celpe", "compesa", "neoenergia", "fatura", "boleto", "segunda via", "vencimento", "débito", "debito", "código de barras", "codigo de barras", "pix copia e cola", "pix copia e cola", "sua conta", "fatura digital"];
  const paymentActions = ["venceu", "vencerá", "vencendo", "pagamento", "quitação", "atraso", "negociar", "desconto", "regularizar", "pendência", "pendencia", "aviso", "notificação", "notificacao", "notificamos"];
  ...
  if (hasUtility && hasAction && (
    textLower.includes("serasa") || 
    textLower.includes("celpe") || 
    textLower.includes("compesa") || 
    textLower.includes("neoenergia") ||
    textLower.includes("notificamos") ||
    textLower.includes("prezado cliente") ||
    textLower.includes("prezada cliente") ||
    textLower.includes("prezado(a) cliente") ||
    textLower.includes("identificamos que") ||
    textLower.includes("evite a suspensão") ||
    textLower.includes("suspensão do fornecimento") ||
    /debito(s)? pendente(s)?/i.test(textLower) ||
    /fatura(s)? em atraso/i.test(textLower) ||
    textLower.includes("corte de energia") ||
    textLower.includes("corte de água")
  )) {
    return { isSpam: true, reason: "Mensagem identificada como aviso de cobrança ou fatura automatizada de utilidade pública" };
  }

  // C. Shortcode phone numbers (usually 4 to 6 digits, e.g., 29202, 27182, etc.)
  if (phone && phone.length >= 4 && phone.length <= 6) {
    return { isSpam: true, reason: `Número de telefone curto ("${fromNumber}"), característico de SMS/WhatsApp corporativo de massa` };
  }

  // D. Standard robot loop patterns
  if (
    textLower.includes("opção inválida") || 
    textLower.includes("opcao invalida") || 
    textLower.includes("digite uma opção") || 
    textLower.includes("digite uma opcao") ||
    textLower.includes("selecione uma opção") ||
    textLower.includes("selecione uma opcao") ||
    textLower.includes("escolha uma das opções") ||
    textLower.includes("escolha uma das opcoes") ||
    (textLower.includes("olá! eu sou") && (textLower.includes("assistente") || textLower.includes("robô") || textLower.includes("robo"))) ||
    (textLower.includes("olá") && textLower.includes("sou o") && (textLower.includes("atendente virtual") || textLower.includes("assistente virtual")))
  ) {
    return { isSpam: true, reason: "Mensagem contém gatilhos típicos de autoatendimento/outro robô (risco de loop infinito)" };
  }
...
async function startServer() {
  ...
  // Helper function to return Brazilian phone number variations (with or without the 9th digit)
  const getBrazilianPhoneVariations = (phone: string): string[] => {
    const clean = String(phone).replace(/\D/g, "");
    const variations = [clean];
    
    // Brazilian mobile number normalization
    // 55 (country) + 2 digits (DDD) + 8 or 9 digits
    if (clean.startsWith("55") && (clean.length === 12 || clean.length === 13)) {
      const ddd = clean.substring(2, 4);
      // If 13 digits, it has a 9th digit. Strip it to get the 12-digit variation.
      if (clean.length === 13 && clean.charAt(4) === "9") {
        const without9 = "55" + ddd + clean.substring(5);
        variations.push(without9);
      }
      // If 12 digits, add the '9' to get the 13-digit variation.
...
2. TIPOS DE REPARO E LIMITAÇÕES TÉCNICAS:
   ...
   - Notebooks e Computadores (Desktops): NÃO realizamos reparos avançados em placas-mãe por razões técnicas (como soldar componentes internos da placa que queimaram). Nossos reparos eletrônicos avançados de placa de nível de componente são voltados única e exclusivamente para a linha de celulares.
   ...
   - IMPORTANTE (Diferença entre reparo e upgrade/substituição): Nós NÃO reparamos os componentes internos de uma placa-mãe de notebook/PC que queimaram, mas FAZEMOS a substituição completa de placas-mãe, instalação de placas novas, upgrade de peças e instalação de kits upgrade (processador, placa-mãe, memória RAM). Se o cliente quer instalar um kit upgrade ou trocar a placa-mãe dele por uma nova, nós FAZEMOS esse serviço com total maestria! Somos uma assistência técnica e este serviço faz parte da nossa especialidade.
...
4. LINHA GAMER E COMPUTADORES/NOTEBOOKS:
   ...
   - Instalação e Upgrade de Hardware (MUITO IMPORTANTE):
     * Nós FAZEMOS a instalação e montagem de kits upgrade (processador, placa-mãe, memória, etc.) em computadores e notebooks. Somos uma assistência técnica e realizamos a montagem/instalação completa desse kit para o cliente de forma profissional.
     * Sobre a venda de kits upgrade: Nós não trabalhamos com estoque direto de kits upgrade, mas temos total viabilidade de cotar e vender o kit para o cliente sob encomenda se ele desejar. Se ele mandar um link de um kit ou perguntar sobre, diga que fazemos a instalação completa e que também temos a viabilidade de cotar e vender o kit sob encomenda!
...
8. Honestidade e Segurança: NUNCA invente informações sobre preços, serviços, políticas, ponto de referência, bairro ou localização física que não estejam descritas de forma clara. Se o cliente perguntar onde fica ou qual o endereço completo, responda EXCLUSIVAMENTE com o endereço físico oficial fornecido ("Travessa das Flores, 167, Salgado - Caruaru - PE"). NUNCA invente outras avenidas, bairros ou pontos de referência (por exemplo, NUNCA diga que fica no "Alto do Moura" ou na "Avenida Agamenon Magalhães" ou "Maurício de Nassau"). Se o cliente perguntar ou mencionar "é no Alto do Moura?" ou outra referência que não seja Travessa das Flores, responda educadamente desmentindo ou esclarecendo que não fica lá, e informe o endereço oficial ("Travessa das Flores, 167, Salgado - Caruaru - PE") de forma completa. Se não souber responder alguma outra dúvida, explique de forma amigável que não possui essa informação no sistema e convide o cliente a aguardar um momento que um atendente humano irá dar mais detalhes.
  ...
  app.post(["/api/webhook/whatsapp", "/webhook", "/api/webhook"], async (req, res) => {
    try {
      ...
      if (messageType === "outgoing") {
        ...
        // In outgoing messages, body.sender is the agent, so we must never use body.sender.phone_number.
        // We prioritize the conversation's contact phone number, then the message's contact object.
        const fromNumber = body.conversation?.contact?.phone_number || 
                           body.contact?.phone_number || 
                           (body.conversation?.contact_inbox?.source_id ? String(body.conversation.contact_inbox.source_id).split("@")[0] : "") || 
                           "";
        const cleanPhone = String(fromNumber).replace(/\D/g, "");

        if (rawMessageId && botSentMessageIds.has(rawMessageId)) {
          console.log(`[Chatwoot Webhook] Mensagem de saída ${rawMessageId} enviada pelo próprio robô. Ignorando silenciosamente.`);
          botSentMessageIds.delete(rawMessageId);
          return res.status(200).send("BOT_REPLY_ACKNOWLEDGED");
        }

        console.log(`[Agent Silence] Mensagem de saída ${rawMessageId} para a conversa ${cleanPhone} enviada por agente humano.`);
        ...
        try {
          ...
          if (enableSilence && cleanPhone) {
            const silencedUntil = Date.now() + (durationMinutes * 60000);
            
            if (db) {
              const variations = getBrazilianPhoneVariations(cleanPhone);
              for (const phoneVal of variations) {
                await setDoc(doc(db, "whatsapp_silence", phoneVal), {
                  silencedUntil: silencedUntil,
                  silencedAt: Date.now(),
                  byMessageId: rawMessageId,
                  conversationId: chatwootConversationId || null
                });
              }
            }
            
            const agentName = body.sender?.name || body.user?.name || "Atendente";
            addWebhookLog('system', `Silenciador de IA Ativado [${durationMinutes}min]`, `Agente humano (${agentName}) respondeu o cliente (${cleanPhone}). O robô ficará em silêncio para esta conversa.`);
          }
        } catch (err: any) {
      ...
      // We prioritize the customer/contact's phone number exactly like in the outgoing handler to avoid mismatches.
      const fromNumber = body.conversation?.contact?.phone_number || 
                         body.contact?.phone_number || 
                         body.sender?.phone_number || 
                         (body.conversation?.contact_inbox?.source_id ? String(body.conversation.contact_inbox.source_id).split("@")[0] : "") || 
                         `cw-${chatwootConversationId}`;
      ...
      (async () => {
        ...
        // --- NEW: Automated Spam and Loop Prevention Check ---
        ...
        if (spamCheck) {
          ...
          addWebhookLog('system', `Mensagem de ${customerName} ignorada (${fromNumber})`, `O robô detectou um sistema automatizado ou padrão de loop infinito e ignorou a mensagem para evitar loops automáticos. Motivo: ${spamCheck.reason}.`);
          if (messageId && db) {
            try {
        ...
        // 3.5 Check if the contact is in temporary Agent Silence mode (human responded recently)
          ...
          try {
            ...
            if (cleanFrom) {
              ...
              for (const val of phoneVariations) {
                const silenceSnap = await getDoc(doc(db, "whatsapp_silence", val));
              ...
              if (isSilenced) {
                ...
                const minutesLeft = Math.ceil((silencedUntil - Date.now()) / 60000);
                ...
                addWebhookLog('system', `Robô Silenciado (Atendimento Humano Recente)`, `O robô não responderá ${customerName} (${fromNumber}) porque um atendente humano respondeu recentemente. Silêncio ativo por mais ${minutesLeft} minutos.`);
                if (messageId) {
                  try {
                    await setDoc(doc(db, "processed_messages", messageId), { 
                      processedAt: new Date().toISOString(),
