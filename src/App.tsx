...
import { defaultConfig as staticConfig } from "./data/defaultConfig";
                ...
                <div className="lg:col-span-2 p-6 rounded-3xl bg-[#0b101d] border border-slate-800/60 space-y-6" id="integration-config-panel">
                  ...
                  <div className="space-y-6" id="integration-forms">
                    ...
                    <div className="space-y-4">
                      ...
                      {/* Silenciamento Automático com Atendimento Humano */}
                      <div className="border-t border-slate-800/40 pt-4 mt-2 space-y-4" id="agent-silence-config-block">
                        <h5 className="text-xs font-bold text-slate-300 font-mono uppercase tracking-wider">Silenciamento Automático</h5>
                        
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="flex items-center justify-between p-3.5 rounded-xl bg-slate-900 border border-slate-800/50" id="container-enable-agent-silence">
                            <div className="space-y-0.5">
                              <label className="text-xs font-semibold text-slate-200">Pausar IA na Resposta Humana</label>
                              <p className="text-[10px] text-slate-400">Silencia o robô temporariamente quando você ou um atendente humano responde no Chatwoot.</p>
                            </div>
                            <button
                              type="button"
                              onClick={() => handleSaveConfig({ enableAgentSilence: !config.enableAgentSilence })}
                              className={`w-10 h-5.5 rounded-full p-0.5 transition-colors cursor-pointer shrink-0 ${config.enableAgentSilence !== false ? 'bg-indigo-600' : 'bg-slate-800'}`}
                              id="btn-toggle-agent-silence"
                            >
                              <div className={`w-4.5 h-4.5 rounded-full bg-white transition-transform ${config.enableAgentSilence !== false ? 'translate-x-4.5' : ''}`}></div>
                            </button>
                          </div>

                          {config.enableAgentSilence !== false && (
                            <div className="p-3.5 rounded-xl bg-slate-900 border border-slate-800/50 flex flex-col justify-center" id="container-silence-duration">
                              <label className="block text-xs font-mono text-slate-400 mb-1.5 flex justify-between items-center">
                                <span>Tempo de Silêncio (Minutos)</span>
                                <span className="text-[10px] text-indigo-400 font-semibold">{config.silenceDurationMinutes || 120} min</span>
                              </label>
                              <input
                                type="number"
                                min="1"
                                max="1440"
                                value={config.silenceDurationMinutes || 120}
                                onChange={(e) => handleSaveConfig({ silenceDurationMinutes: parseInt(e.target.value) || 120 })}
                                className="w-full px-3 py-1.5 rounded-lg bg-slate-950 border border-slate-800 text-slate-200 text-xs focus:outline-none focus:border-indigo-500 font-mono"
                                id="input-silence-duration-min"
                              />
                            </div>
                          )}
                        </div>
